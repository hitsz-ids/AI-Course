...
# 在 evaluation/backtest.py 文件开头添加
...
# Must be imported before pickle.load to allow deserialization of FileLazyTSDataset
# (the pkl was created when prepare_data.py ran as __main__, so the class is stored
#  as __main__.FileLazyTSDataset; importing it here makes it available in __main__)
from prepare_data import FileLazyTSDataset  # noqa: F401
...
# ========== 路径配置 ==========
OUTPUT_DIR = 'output'  # 输出根目录，可在此统一修改
...
def main(stage, universe, exp_no, seed=0, ensemble=False):
    ...
    # data_dir = 'dev_v1/data'
    # model_dir = 'dev_v2/model'
    if universe == 'csi300':
        benchmark = "SH000300"
    ...
    model_config = ModelConfig(
        universe=universe,  # csi300或 'csi800'
        prefix='opensource',  # 或 'original'
        seed=seed,
        model_dir=model_dir,
        data_dir=data_dir,
    )
    ...
    backtest_config = BacktestConfig(
        start_time=start_time,
        end_time=end_time,
        qlib_data_dir="/workspace/dataset/qlib_bin",  # 使用正确的数据路径
        region="cn",
        topk=10,    # 论文中topk=30
        n_drop=2,  # 论文中n_drop=30
    ...
    try:
        ...
        qlib.init(provider_uri="/workspace/dataset/qlib_bin", region="cn")  # 使用正确的数据路径
...
if __name__ == "__main__":
    ...
    parser.add_argument('--stage', type=str, choices=['reproduce', 'dev', 'dev_bear', 'production'], 
    ...
    parser.add_argument('--ensemble', action='store_true', default=True, help='ensemble')
    parser.add_argument('--no-ensemble', dest='ensemble', action='store_false', help='disable ensemble')
